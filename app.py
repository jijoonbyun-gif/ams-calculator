...
if st.button("비중 계산하기"):
        ...
        if data.empty:
            ...
            # [수정 핵심] MultiIndex 컬럼을 평면화하여 'Adj Close'에 직접 접근할 수 있게 만듭니다.
            if isinstance(data.columns, pd.MultiIndex):
                data.columns = data.columns.get_level_values(0)
            
            # 'Adj Close'가 없는 경우를 대비해 'Close'를 대안으로 사용합니다.
            target_col = 'Adj Close' if 'Adj Close' in data.columns else 'Close'
            prices = data[target_col].dropna()
            
            # 데이터 개수 확인 (최소 13개월 필요: 현재 + 과거 12개월)
            if len(prices) < 13:
                st.error(f"분석에 필요한 데이터가 부족합니다. (현재 {len(prices)}개월치 확보)")
            else:
